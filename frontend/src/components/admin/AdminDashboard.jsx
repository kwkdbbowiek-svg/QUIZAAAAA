...
export default function AdminDashboard() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setErr('Yuklab bo\'lmadi: ' + e.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (err) return (
    <div className="card text-center py-6 space-y-3">
      ...
      <button onClick={load} className="btn-primary px-4 py-2 text-sm">🔄 Qayta</button>
    </div>
  )

  const cards = [
    { icon: '👥', label: 'Jami userlar',     value: stats?.total_users ?? 0,        color: 'from-blue-600 to-blue-400' },
    ...
    { icon: '🏆', label: 'Faol challenge',   value: stats?.active_challenges ?? 0,  color: 'from-yellow-600 to-yellow-400' },
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        ...
        <button onClick={load} className="text-blue-400 text-sm">🔄</button>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {cards.map(c => (
          <div key={c.label} className={`rounded-2xl p-4 bg-gradient-to-br ${c.color} shadow-lg`}>
